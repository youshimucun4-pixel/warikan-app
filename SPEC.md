...
| 招待リンク | **10分間有効のランダムURL**を発行可能。リンク経由ならコード入力なしで参加 |
...
- **ふたり + Firebase の名前入力:** 作成者は「あなたの名前」のみ入力して部屋作成。参加者は合言葉入力または招待リンク経由で「あなたの名前」を入力し、未設定側メンバー名として登録される。
- **グループ + Firebase の名前入力:** 作成者は「あなたの名前」でグループ作成。招待された参加者は参加時に自分の名前を入力してメンバーに追加される。
...
rooms/
  {roomCode}/
    ...
    mode: 'group'
    members: [{ id: string, name: string, uid?: string }, ...]  // 1-10人（招待で増加）
    memberUids: [uid, ...]
...
[ホーム画面]（ログイン済み、または Firebase 未使用時）
  ...
  └─ ＋ 新しいグループ → [セットアップ画面]
                           ...
                           ├─ ふたり + ローカル → 直接作成
                           ├─ グループ + Firebase → 選択 → 作成(作成者名を入力) or 参加(参加者名を入力)
...
- グループモード（Firebase）のメンバー名は、作成者・参加者がそれぞれ参加時に自分で入力する
